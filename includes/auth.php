...
function registerUser($data)
{
    try {
        ...
        try {
            // Insert user
            $stmt = $pdo->prepare("INSERT INTO users (username, email, password, sponsor_id) VALUES (?, ?, ?, ?)");
            $stmt->execute([$data['username'], $data['email'], $hashed_password, $sponsor_id]);
            ...
            logEvent("New user registered: " . $data['username'], 'info');
